...
{
    int i,j,num=1;
    for(i=2;i<=61234;i++)///这个六万多是预估的，根据大神说，越到后面删除的数应该越少，因此被遍历的数应该不是很大(写到后面才知道因为后面的幸运数字很大，那么他的倍数(删除数)也被拉到很后面去了)
    {///外层循环，寻找从2号到n号每个的倍数
        if(vis[i]==false)///先看正在遍历的这个数是不是被删除的
        {
            ...
            {
                if(vis[j]==false)t++;///如果从幸运数字往后+1的这个数字(j)没有被删除，那么它就是可以计数的，此时计数器t将其算入
