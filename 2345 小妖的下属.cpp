...
int main()///(st+(st+d))*(d+1)/2   化简后求st=(2*n-d*d-d)/(2*d+2) 枚举d值，从0到sqrt(2*n)，因为分子是用2*n来减d*d因此d不会超过sqrt(2*n)
{
    int n;
    while(scanf("%d",&n)!=EOF)
    {
        ...
        for(int i=0; i<=up; i++)
            if((2*n-i*i-i)%(2*i+2)==0)///注意，此时会解出起始值是0或n的情况，这样只有一个数或有人为0的分法不算答案
