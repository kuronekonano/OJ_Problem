...
int main()
{
    ...
    while(t--)
    {
        ...
        int sum=0;
        for(int i=0; i<n; i++)
            if(v[maxi].find(v[i])!=string::npos) sum++;///string中find函数应用，判断v[i]是否是v[maxi]的子串，是返回地址，否则返回string::npos
        if(sum==n)cout<<v[maxi]<<endl;
        else cout<<"No"<<endl;
    }
}
//#include <stdio.h>
...
//char *t[1100010],*s;///每个字符串在总字符串s中的位置，用*t表示，因为有哦很多个字符串，因此用数组表示每一个字符串的位置，相当于模拟了二维数组，只不过每个数字长度可变
//int main()
//{
//    int T,n;
//    scanf("%d",&T);
