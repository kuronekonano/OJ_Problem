...
int main()
{
    ...
    while(scanf("%d%d%d",&t,&p,&s)!=EOF)
    {
        ...
        for(int i=0; i<s; i++)
        {
            if(sss[i].out[1]=='Y')
            {
                if(!team[sss[i].tt])tmin--;///是否所有队做出题
                if(!pro[sss[i].pp[0]-'A'])pmin--;///是否所有题都被A
                if(!tp[sss[i].tt][sss[i].pp[0]-'A'])///已经A过的题可能又交了，这时是不能重复计数的
                {
                    team[sss[i].tt]++;///每队几题
                    pro[sss[i].pp[0]-'A']++;///每题几队
                    if(pro[sss[i].pp[0]-'A']>pmax) pmax=pro[sss[i].pp[0]-'A'];///防所有队做出某题
                    if(team[sss[i].tt]>tmax) tmax=team[sss[i].tt];///防AK
                }
                tp[sss[i].tt][sss[i].pp[0]-'A']=true;
            }
//            printf("-------> pmax=%d tmax=%d pmin=%d tmin=%d\n",pmax,tmax,pmin,tmin);
            if(!flag&&tmin==0&&pmin==0&&pmax!=t&&tmax!=p)///判断是否符合开始时间
            {
                sth=sss[i].hh,stm=sss[i].mm,sts=sss[i].ss;///pmax作为完成人数最多的题目计数，当有t人写完某题时，说明这题被所有人写出来
