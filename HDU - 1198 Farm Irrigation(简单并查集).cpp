...
int main()
{
    while(scanf("%d%d",&n,&m)!=EOF)
    {
        ...
        for(int j=1;j<=n*m;j++)z[j]=j;
        for(int i=1;i<=n;i++)
        {
            for(int j=1;j<=m;j++)
            {
                if(i!=1)join(i,j,i-1,j);
                if(j!=m)join(i,j,i,j+1);
                if(i!=n)join(i,j,i+1,j);
                if(j!=1)join(i,j,i,j-1);
            }
        }
        int ans=0;
        for(int i=1;i<=n*m;i++)if(z[i]==i)ans++;
        printf("%d\n",ans);
    }
}
