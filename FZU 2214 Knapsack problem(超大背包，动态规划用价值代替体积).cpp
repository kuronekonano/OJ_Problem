...
int main() {
  ...
  while (t--) {
    ...
    for (i = 1; i <= n; i++) {
      ...
      wsum =
          wsum +
          a[i].w; /// 记录最大总体积，用于初始化dp数组中元素，因为是找能达到v的最小体积，因此初始化都最大的值
    }
    for (i = 1; i <= vsum; i++) {
      dp[i] = wsum;
    }
    ...
    for (i = 1; i <= n; i++) /// 从第1个物品开始
    {
      for (
          j = vsum; j >= a[i].v;
          j--) /// 最大价值开始向前遍历，直到遍历第i个物品的价值，说明达到这个物品价值的体积，就是这个物品的体积
      {
    ...
    for (
        i = vsum; i >= 0;
        i--) /// dp完成后，将从最大价值回头遍历，查找当dp[i]==背包容积时，所能达到的最大体积(也就是v体积时使用的最小体积)
    {
      if (dp[i] <= b) {
        ans = i;
        break;
