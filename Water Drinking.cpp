...
{
    int i,n,a,b,first;
    int link[100008],head[100008];///骆驼排队喝水，那么最短的那排的最后一个编号是什么
    while(scanf("%d",&n)!=EOF)///有n只骆驼
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            if(a==0)head[first++]=b;///当关系是与0相接时，说明是那一排的第一个，或是说，出现了新的一排，head根节点数组增加一位，表示多了一列队伍
            else link[a]=b;///否则只是普通的两两之间连接关系(排队)
        }
