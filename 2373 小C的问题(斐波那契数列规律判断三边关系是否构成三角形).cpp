...
#define LL long long///由于数列是乱序的，数列中每个值小于1e18，数列长度和区间询问次数都是1e5，不可能暴力查询
using namespace std;///此处想到，因为两边之和一定要大于第三边，因此当数列满足，排序后，较小两边相加都小于等于下一条边，这种序列即斐波那契数列，甚至比斐波那契数列更容易增大的数列
struct node///反过来想想，既然构造一个超长的不能满足构成三角形的数列必须最低限度是构造一个斐波那契数列，那么这个数列长度能否构造得很长？不行，因为斐波那契数列到第100个就超过1e10了
