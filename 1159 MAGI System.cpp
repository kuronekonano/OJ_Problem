...
//        int la=strlen(a);///统计长度，其中，最高位在数组的0位置，个位在la-1的位置
//        int lb=strlen(b);
//        int cnt=0;///注意这里，因为字符数组是从最后一位开始读入的，而数字数组是从0位开始存的，所以赋值过去时是两个单独的计数，必须分开
...
//        }///计算部分结束
//        ///输出部分
//        for(int i=1004; nc[i]==0; i--)///倒着从数组最后一位找，循环条件是只要这一位是空的（为0）就继续下去
//        {///当找到i位的下一位不为零！就说明从下一位开始输出这串数字
//            if(nc[i-1]!=0)///从最高位输出，也就是nc结果数组的存储的最后的那个位置
//            {
//                for(int j=i-1; j>=0; j--)///新循环，从数组的下一位一直输出到0位（个位）
//                    printf("%d",nc[j]);///每位输出
