...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    ...
    for (i = 1; i <= num; i++) {
      for (j = i + 1; j <= num; j++) {
        if (a[i].x1 < a[j].x1 && a[i].y1 < a[j].y1 && a[i].x2 > a[j].x2 &&
            a[i].y2 > a[j].y2) {
          ans = max(ans, a[i].s);
          continue;
        }
        if ((a[i].x1 >= a[j].x1 && a[i].x1 <= a[j].x2 && a[i].y1 >= a[j].y1 &&
             a[i].y1 <= a[j].y2) ||
            (a[i].x2 >= a[j].x1 && a[i].x2 <= a[j].x2 && a[i].y1 >= a[j].y1 &&
             a[i].y1 <= a[j].y2) ||
            (a[j].x1 >= a[i].x1 && a[j].x1 <= a[i].x2 && a[j].y1 >= a[i].y1 &&
             a[j].y1 <= a[i].y2) ||
            (a[j].x2 >= a[i].x1 && a[j].x2 <= a[i].x2 && a[j].y1 >= a[i].y1 &&
             a[j].y1 <= a[i].y2))
          continue;
        //                printf("=======%d %d   %d %d
