...
long long gcd( long long a,
               long long b )  /// 如果打电话不存在环，或者有某人不在环中，成了一条直线，那就输出-1
{
    while ( b != 0 )  /// t的意思是
...
int main() {
    ...
    while ( scanf( "%lld", &n ) != EOF ) {
        memset( vis, false,
                sizeof( vis ) );  /// 用标记数组标记某个节点是否走过，如果走过就不用再从这个点遍历他在哪个环里了
        for ( i = 1; i <= n; i++ ) {
            scanf( "%d", &a[ i ] );
        }
        ...
        for ( i = 1; i <= n; i++ )  /// 从第一个人开始检查环
        {
            ...
            if ( flag )
                break;
            if ( cnt % 2 == 0 )
                cnt /= 2;  /// 注意，这个t值是从x->y的长度，也是y->x的长度，如果是奇数长度环，那么x=y，t必为环长度，如果是偶数环，那么从这个环中间的那个人结束，再从他开始，就是t的值，所以要/2表示t
            sum = sum * cnt / gcd( cnt,
