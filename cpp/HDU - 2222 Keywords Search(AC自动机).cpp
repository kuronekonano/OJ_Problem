...
int match( char S[] )  /// 对比查询函数
{
    ...
    {
        if ( Tree[ now ].next[ S[ i ] - 'a' ] )
            now = Tree[ now ].next[ S[ i ] - 'a' ];  /// 类似字典树的匹配，如果当前字符符合字典树的节点字符，往下继续匹配
        else                                         /// 如果失配，那么从失配节点找他的fail，也就从字典树中的某个单词，跳到另一个单词上继续匹配。这要使别的单词的前缀和失配单词的最大前缀相符。
        {
            ...
            while ( tmp != -1 && Tree[ tmp ].next[ S[ i ] - 'a' ] == 0 )
                tmp = Tree[ tmp ].fail;  /// 找到一个符合条件的fail，符合条件的fail既要前缀相同，而且前缀的下一个字母(子节点)要符合串T正在匹配的那个字母
            if ( tmp == -1 )
                now = 0;  /// 如果向上找fail没有，那么只能跳到根节点。
            else
                now = Tree[ tmp ].next[ S[ i ] - 'a' ];  /// 否则now跳到了字典中另一个单词的节点中。
        }
        if ( Tree[ now ].cnt )
            ans += sum( now );
    }
