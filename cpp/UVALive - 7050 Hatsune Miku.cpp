        ...
        for ( i = 2; i <= m; i++ ) {
            ...
            if ( b[ i ] != -1 ) {
                ...
                if ( b[ i - 1 ] != -1 ) {
                    ...
                    for ( j = 1; j <= n; j++ ) {
                        dp[ i ][ tmp ] = max( dp[ i ][ tmp ],
                                              dp[ i - 1 ][ j ] + a[ j ][ tmp ] );  /// 因此dp的坐标是[i]当前遍历位置[tmp]已知坐标，而当当前未知时，才会用j去遍历求每一个的权值
                    }
                }
            }
            else {
                if ( b[ i - 1 ] != -1 ) {
                    for ( j = 1; j <= n; j++ ) {
                        dp[ i ][ j ] = dp[ i - 1 ][ b[ i - 1 ] ]
                                       + a[ b[ i - 1 ] ][ j ];  /// 要注意的点：当上一个已知时，要计算出所有的下一个，此时不需要max筛选，因为上一个用已知的，下一个因为是为止，所以算出每一个就好
                    }
                }
                else {
                    for ( j = 1; j <= n; j++ ) {
                        for ( int k = 1; k <= n; k++ ) {
                            dp[ i ][ j ] = max( dp[ i ][ j ],
                                                dp[ i - 1 ][ k ] + a[ k ][ j ] );  /// 而当上一个未知时，要用max从上一个中选择一个最大值来配下一个，无论下一个已知还是为止，max筛选的是未知的上一个
                        }
                    }
                }
            }
