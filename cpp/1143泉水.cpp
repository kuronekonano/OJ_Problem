...
int dfs( int x, int y )  // DFS函数用于移动坐标
{
    ...
    if ( !vis[ x ][ y ] )  // 若投入坐标未被标记，标记这个坐标，并增加流动面积，一开始的泉眼位置是未被标记的
    {
        area++;
        vis[ x ][ y ] = 1;
    }
    for ( int i = 0; i < 4; i++ )  // 从投入坐标的位置开始行走，4个方向
    {
        ...
        int ny = y + walky[ i ];
        if ( nx >= 1 && nx <= n && ny >= 1 && ny <= m && high[ nx ][ ny ] <= high[ p1 ][ p2 ] )  // 若这个新坐标位置符合条件，未被标记且不出地图，且高度不高，则选择从这里走，进入下一个DFS函数中
        {
            dfs( nx, ny );  // 这是一层递归关系
        }
    }
...
int main()  /// 主函数
{
    while ( scanf( "%d%d%d%d", &n, &m, &p1, &p2 ) != EOF )  // 输入地图方阵大小，输入泉眼位置
    {
