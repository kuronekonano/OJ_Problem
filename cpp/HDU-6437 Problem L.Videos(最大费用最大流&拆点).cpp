...
int main() {
  ...
  while (t--) {
    ...
    int st = 0, ed = (m << 1) + 1, S = (m << 1) + 2,
        T = (m << 1) +
            3; /// 因此每个视频被拆成两个节点，因此总结点数达到了m*2，再加上超级源点和超级汇点
    addedge(st, S, k, 0);
    addedge(T, ed, k, 0);
    for (int i = 1; i <= m; i++) {
      ...
      addedge(S, i, 1, 0);
      addedge(i, i + m, 1,
              -val); /// 因为有时间的限制，因此将一个视频拆成开始时间和结束时间
      addedge(i + m, T, 1, 0);
      for (
          int j = 1; j < i;
          j++) /// 在输入的同时比较其他视频的开始时间，将结束时间小于开始时间的建一条有向边，容量为1，一个人只能看一个视频
        if (to[i] <=
            from[j]) /// 再判断是否是同种视频，是同种则边权为负，否则为0
