...
int  insert( char str[],
    ...
    for ( int i = 0; i < len; i++ ) {
        ...
        if ( tre[ rt ][ x ] == 0 ) {
            tre[ rt ][ x ] = ++tot;
            memset( tre[ tre[ rt ][ x ] ], 0,
                     sizeof( tre[ tre[ rt ][ x ] ] ) );  /// 有t次输入，因此不清空树会造成混乱，因此每次申请新的节点要清空当前节点的孩子
        }
        rt = tre[ rt ][ x ];
        if ( vis[ rt ] == true )
            ans = false;  /// 预防短的号码先出现，在建长号码的树时时刻检查当前路过的节点rt是否曾被作为结束位置
    }  /// 不用排序，在两处判断即可，注意这里是rt<tot!!
    if ( rt < tot )
        ans = false;   /// 预防长的号码先出现，如果这个短号码的的最终结束节点是曾被长串走过的一个节点，也就是说这个节点上标记的号数小于最大节点号数
