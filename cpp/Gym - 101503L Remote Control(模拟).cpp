...
int main() {
  ...
  while (~scanf("%d", &a[1])) {
    ...
    scanf("%d%d%d%d%d%d%d%d%d%d%d%d", &a[2], &a[3], &a[10], &a[4], &a[5], &a[6],
          &a[11], &a[7], &a[8], &a[9], &a[12], &a[0]);
    scanf("%d%d", &n, &m);
    ...
    if (m < 10) /// 目的位置为个位
    {
      ...
      if (!judge(m) &&
          a[10]) /// 不能自己直接跳，就先跳至最近的台，然后只按前进或后退
      {          /// 跳跃后向前
        int tmp = m, sum = 0;
        while (
            !judge(tmp) &&
            sum <
                101) /// 如果根本不能通过这种途径到达，那么这个循环会跳不出去，因此要设定一个最大步数跳出循环
        {
          tmp--;
          sum++;
          if (tmp < 0)
        ...
        int num =
            tmp < 10
                ? 1
                : 3; /// 注意如果最近的台是个位，那么只用按一次操作，否则要按3次
