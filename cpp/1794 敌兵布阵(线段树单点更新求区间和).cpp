...
void build(int l,int r,int root)///建树
{
    ...
    {
        int
mid=(l+r)/2;///如果不是递归终点，找出左儿子和右儿子节点的范围，继续往下二分区间
...
void sub(int i,int subs,int root)///单点更新(减)
{
    ...
    if(i>(a[root].l+a[root].r)/2)
sub(i,subs,root*2+1);///如果不是叶子节点(当指定营地i的值，大于mid时，找右儿子)，也就是继续递归给下一个节点(右儿子的节点)中存储的和减人数，并继续递归
...
void query(int l,int r,int root)///查询
{
    ...
    {
        int mid =
(a[root].l+a[root].r)/2;///这里是三种情况，当左值(较小值)都大于中点mid，说明完全是在右儿子区间范围的
        if(l>=mid+1)///右儿子
        {
            query(l,r,root*2+1);
        }
...
///总结: 情况1：三种情况if else
if方法：递归出口采用==卡位：传参时第三种分叉情况必须传参mid和mid+1
///      情况2：三种情况 if else
if方法：递归出口采用<=包含：传参可以是mid和mid+1,也可以是传参l r
