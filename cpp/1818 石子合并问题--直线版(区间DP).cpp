...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    ...
    for (
        len = 2; len <= n;
        len++) /// dp区间长度，从长度为2的区间相加到长度为n的区间，一直筛选最优值
    {
      for (i = 1; i <= n - len + 1;
           i++) /// 在某个len的长度下，从区间i到i+长度=j这段距离
      {
        j = i + len - 1;
        for (
            k = i; k < j;
            k++) /// 在i到j这段长len的距离下，取某个中点k作分成两份，从而筛选两堆求和的最优值
        {
          dpmax[i][j] = max(
              dpmax[i][j],
              (dpmax[i][k] + dpmax[k + 1][j] + sum[j] -
               sum[i -
                   1])); /// 也就是区间i到k，和区间k+1到j之和。加上求和后的区间总值
