...
void cal_dis( int rt ) {
    if ( tre[ rt ].cnt > 0 )
        tre[ rt ].dis = tre[ rt ].ren - tre[ rt ].len;  /// 若覆盖计数大于0，该区间的覆盖面积为区间长度
    else if ( tre[ rt ].r - tre[ rt ].l == 1 )
        tre[ rt ].dis = 0;  /// 叶子节点
    else
        tre[ rt ].dis = tre[ rt << 1 ].dis + tre[ rt << 1 | 1 ].dis;  /// 不是叶子节点，通过向上传递，两个子节点的dis之和即更大区间的dis
    return;
}
void update( edge line, int rt ) {
    ...
    if ( line.yu <= tre[ rt << 1 ].ren )
        update( line, rt << 1 );
    else if ( line.yd >= tre[ rt << 1 | 1 ].len )
        update( line, rt << 1 | 1 );
    else {
        edge tmp = line;
        tmp.yu   = tre[ rt << 1 ].ren;
