...
struct Splay_tree
{
    ...
    void pushdown(int x)///向下传递
    {
        ...
        if(tre[x].rev)
        {
            if(tre[x].son[0]) tre[tre[x].son[0]].rev^=1;
            if(tre[x].son[1]) tre[tre[x].son[1]].rev^=1;
...
//    }
    ...
    int select(int pos)
    {
        ...
        {
            if(pos<tre[tre[u].son[0]].siz) u=tre[u].son[0];
            else pos-=tre[tre[u].son[0]].siz+1,u=tre[u].son[1];
            pushdown(u);
        }
        return u;
    }
    ...
    void init(int n)///初始化
    {
        ...
        tre[1].init(-inf);
        tre[n+2].init(-inf);
        for(int i=2; i<=n+1; i++) tre[i].init(0);
