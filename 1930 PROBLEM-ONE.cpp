...
//    {
//        strc[lenr-i]=result[i];///或是【存储输出数组】的最后一位（最高位），接收【计算数组】的第0位（结束标识符\0）    【一样的！！！】
//    }
//}
//int main()
//{
//    char c[1005][1005];///只需要一个字符串二维数组，有1000个斐波那契数，每个数有最大1000位
//    int t,n;
//    scanf("%d",&t);///t组数据
//    while(t--)
//    {
...
#include<string.h>
int a[1005][1008];///一个int数组存储大数即可，有1000个斐波那契数，每个不超过一千位
int main()
{
    int i,j,n,t;
    scanf("%d",&t);///t组数据
    while(t--)
    {
        ...
        int num=2;///num表示每个数的位数+1，因此初始的数是1，那么位数加1初始就是2
        int sum=0;///每个数位的求和数
        int ans=0;///进位存储
        for(i=3; i<=n; i++)///从第三个斐波那契数开始求，求到第n个
        {
            for(j=1; j<num; j++)///每个数从数位的个位开始求，存储时，第1位是个位，最高位存储的是最高位，末尾输出时是倒着从最高位输出到第1位的
            {
                sum=a[i-1][j]+a[i-2][j]+ans;///大数计算过程，取数组中上一个数和上上一个数的j位依次相加到最高位，每位的和为sum
                a[i][j]=sum%10;///其实第i个斐波那契数的每个j位都是每个数位求和后取摸
                ans=sum/10;///进位的数是求和数除10
            }
            while(ans)///若进位在上面的循环中还存在，那么说明总位数num还在增加
            {
                a[i][num++]=ans%10;///大数计算过程，理论上进位时必须模10取个位，但此处因为是斐波那契数列加法计算，因此必定不会超过进一位，可以不用模10
