...
int main()
{
    ...
    while(t--)
    {
        ...
        sort(each,each+n,cmp);//作业按时间排序，注意若期限相同，优先取分数扣的多的在前面
...
///其中扣分流程有一环节可以使目前不能完成的作业，找到已经可以完成的作业来替换，若不能完成的作业扣分远大于已经完成的某项作业，可以使那份扣分较少的作业代替扣分多的作业
            ...
            for(j=0; j<i; j++)//从之前已经判断可以完成的作业中找是否存在可以以更小分数为代价，代替目前目前作业扣分的作业，因为是循环，所以最小扣分是一直在更新的
            {
                if( each[j].mark < k && each[j].finish)//若之前的作业所扣分数小于当份作业，且能完成
                {
