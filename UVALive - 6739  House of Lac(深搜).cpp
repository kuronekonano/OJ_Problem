...
void dfs(int ii,int jj)
{
    ...
    for(int i=0; i<4; i++)
    {
        ...
        if(!vis[nexti][nextj]&&ma[nexti][nextj]=='.'&&nexti>=1&&nextj>=1&&nexti<=n&&nextj<=m)
        {
        ...
        if(ma[nexti][nextj]=='*')flag[nexti][nextj]++;///接触计数
    }
    for(int i=0; i<4; i++)///对于消去的*，可以立即从该点搜索下去
    {
        ...
        {
            if(nexti==ed1&&nextj==ed2)///消去的点若是终点，可以立即得到ans并返回
            {
                ans=true;
                return ;
            }
...
int main()
{
    ...
    while(t--)
    {
        ...
        if(st1==ed1&&st2==ed2)ans=true;///起始点等于终点则不用搜索
        if(!ans)dfs(st1,st2);
        printf("%s\n",ans?"YES":"NO");
    }
}
