...
int main()
{
    ...
    {
        scanf("%d",&n);
        for(int i=0;i<=n+1;i++)v[i].clear();///一定要清表！！！！！否则会超时，形成一个多次测试数据拼成的超大图
        for(int i=1;i<=n;i++)
        {
            scanf("%d",&city);///输入城市点权
            v[0].push_back(edge(i,city));///虚拟起电脑设为0号点，将所有点的取值连接到起始节点，让起始节点选择应该从哪个城市买书(出发)
            v[i].push_back(edge(n+1,-city));///虚拟终点设为n+1号点，终点投入所有点能到达的点中，使最短路中可以在每个点都能选择到达终点！
        }
        int from,too,vv;
        for(int i=1;i<=n-1;i++)
        {
