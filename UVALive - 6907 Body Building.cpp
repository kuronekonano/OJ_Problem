        ...
        for(int i=1; i<=n; i++)
        {
            if(!vis[i])
            {
                ...
                for(int j=1; j<=n; j++)///在标记的节点中进行节点数量计数
                    if(dvis[j])
                        for(int k=1; k<=n; k++)
                            if(ma[j][k])edge++;///对所有被标记的点，作为这个连通图的一部分，记录这些点连成的边的数量
                edge/=2;
                if(node&1)continue;
                if(edge==(node/2)*(node/2-1)+1)///多边形有n个点，那么就有((n/2-1)*n)/2条边的完全图，这里将node个点分成两份，因此共有node/2*(node/2-1)条边，加上两个哑铃相连的一条边
                {
                    int out[2]={0,0},num=0;///两个点的位置要初始化，否则后果不堪设想！！
                    for(int j=1; j<=n; j++)
                    {
