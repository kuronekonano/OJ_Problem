...
int main()
{
    ...
    for(int i=1; i<=cnt; i++) sort(pos[i].begin(),pos[i].end());
    scanf("%d",&m);
    bool flag=true;
    int num=0,tp;
    for(int i=1; i<=m; i++)
    {
        cin>>b[i];
        tmp=mp[b[i]];
        if(i==1) tp=pos[tmp][0],vis[i]=tp;
        if(i!=1&&flag)
        {
            num= lower_bound(pos[tmp].begin(),pos[tmp].end(),tp) -pos[tmp].begin();
            if(num==pos[tmp].size()) flag=false;
            else tp=pos[tmp][num],vis[i]=tp;
        }
    }
    if(!flag)
    {
    ...
    for(int i=m-1; i>=1; i--)
    {
        tmp=mp[b[i]];
        num=lower_bound(pos[tmp].begin(),pos[tmp].end(),tp)-pos[tmp].begin();
        if(num==pos[tmp].size())num--;
        if(tp<pos[tmp][num])num--;
        tp=pos[tmp][num];
        if(tp!=vis[i])
        {
            printf("ambiguous\n");
