...
{                   ///当乘法过大时，从加法位集体向后挪一位给除法位，再计算取最大值，这种情况分给乘除4位，乘法各一位，除法2位
    int t,i,cas=0;  ///最后一种，当两位乘法乘积仍大于后两位除法时，继续从加数中集体后挪一位给除法，最后除3位肯定是小于1的，分给乘除5位，乘法各一位，除法分3位
    scanf("%d",&t); ///如果是第一种情况，5位数必出结果，如果是第二种情况，6位数二选一取最大出结果，超过6位，在第一第二种中最大值再与第三种情况取max，得到结果
    while(t--)
    {
        cas++;
        char str[25];
        ...
        for(i=0;i<l-4;i++) numa=numa*10+a[i];
        numa+=a[l-4];
        for(i=1;i<l-3;i++) numb=numb*10+a[i];
        numb+=a[0];
        ...
        numa=0,numb=0;
        for(i=0;i<l-5;i++) numa=numa*10+a[i];
        numa+=a[l-5];
        for(i=1;i<l-4;i++) numb=numb*10+a[i];
        numb+=a[0];
        ...
        numa=0,numb=0;
        for(i=0;i<l-6;i++) numa=numa*10+a[i];
        numa+=a[l-6];
        for(i=1;i<l-5;i++) numb=numb*10+a[i];
        numb+=a[0];
        sum2=max(numa,numb);
