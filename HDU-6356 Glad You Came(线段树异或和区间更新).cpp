...
void pushup(int rt)///那么线段树维护区间最小值，这样更新时方便查找哪段区间是小于更新值的
{
    tre[rt].val=min(tre[rt<<1].val,tre[rt<<1|1].val);
}
void pushdown(int rt)///延时更新传递更新最大值，保证多次更新后维护到底层用的是有效的最大值更新
{
    if(tre[rt].val)
    {
...
{
    if(tre[rt].val>=val)return;
    if(tre[rt].l>=l&&tre[rt].r<=r)///更新时要求该区间最小值小于被更新值，那么所有的最小值都会被更新，与区间内最大值无关
    {
        if(tre[rt].val<=val)tre[rt].val=val;
        return ;
    }
    pushdown(rt);
