...
int querymax(int l,int r,int rt)
{
    ...
    {
        int mid=(tre[rt].l+tre[rt].r)>>1;
        if(mid<l)return querymax(l,r,rt<<1|1);
        else if(mid>=r)return querymax(l,r,rt<<1);
        else return max(querymax(l,r,rt<<1|1),querymax(l,r,rt<<1));
    }
}
int querymin(int l,int r,int rt)
{
    ...
    {
        int mid=(tre2[rt].l+tre2[rt].r)>>1;
        if(mid<l)return querymin(l,r,rt<<1|1);
        else if(mid>=r)return querymin(l,r,rt<<1);
        else return min(querymin(l,r,rt<<1|1),querymin(l,r,rt<<1));
    }
}
int main()
{
