...
int main()
{
    ...
    while(scanf("%d",&n)!=EOF)
    {
        ...
        for(i=1; i<=n; i++)
        {
            for(j=i; j<=2*n-i; j++)///用英子的方法，将坐标四角缩小2*n-i能实现右边和下面的坐标相应缩小
            {
                for(k=i; k<=2*n-i; k++)///列
                {
                    a[j][k]=(n-i)%2==0?'*':'.';///拿外层的第层来判断是‘.’还是‘*’    最后判断奇偶就好
//                    printf("j=%d k=%d s=%d\n",j,k,s);
                }
            }
        }
...
//            {
//                int  a=fabs(i-n);///处理i和j，做差求绝对值后，可得到各个坐标离中心点的距离，同时，这个距离也是层数数值越大，层数靠外
//                int  b=fabs(j-n);///偶数层属于 . 奇数层属于 * ,但是，又因为，a与b计算的是与中心的距离，因此层数是从0开始的，所以偶数层0 2 4就是(*)，奇数层1 3 5就是(.)
