...
double cal(int r, int R, int x1, int x2, int y1, int y2) {
  ...
  if (dis > R - r && dis < R + r) /// 相交
  {
    double S1 = Pi * r * r * 2 *
                ...
                (2 * Pi); /// 余弦定理，三角形三边算cos值
    double S2 = Pi * R * R * 2 *
                acos((R * R + dis * dis - r * r) / (2 * R * dis)) / (2 * Pi);
    double S3 =
        sin(acos((R * R + dis * dis - r * r) / (2 * R * dis))) * R *
        dis; /// 菱形面积，垂直的对角线长度除2,垂直于圆心距的对角线用sin值来求长度
...
int main() {
  ...
  while (t--) {
    ...
    double area = cal(R, R, x1, x2, y1, y2) - 2 * cal(r, R, x1, x2, y1, y2) +
                  cal(r, r, x1, x2, y1, y2); /// 容斥原理
    printf("Case #%d: %.6lf\n", cas, area);
  }
}
