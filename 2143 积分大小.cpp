...
{///分别用新加入的这个数n来替换两个数中的任一一个，在2个不同的位置中更换n值进去，即有[n-1][2]*2种情况，相当于每种情况可以更换2个数各一次，有[n-1][2]种情况要更换
    long long ans[20],dp[22][22],i,j,n;///这是在n个数增加时，有同等个数的不同大小值时的状态转移，利用新数n对n-1个数时同样数量的不同值更换得到
    memset(dp,0,sizeof(dp));///另一种情况的转移，是通过n-1个数时，有j-1个不同大小的数时的情况数量递推得到，使新加入的这个数n作为和之前所有的数值都不同的存在加入到数列中
    memset(ans,0,sizeof(ans));///它可以放在原数列从大到小排列时的任何位置，任意两个不同大小的数之间，也能比所有的数大，比所有的数小，在j-1个不同的值中有j个空隙可以填充
