...
int digui(int n,int x,int y,int h,int w)
{
    ...
    int nh=pow(2,n-2)/2,nw=(pow(2,n-1)-1)/2;///上一个三角形坐标，各点坐标
    digui(n-1,x,y,nh,nw);///右下角，以三角形右下角为标准点，分支扩展
    digui(n-1,x,y-w-1,nh,nw);///左下角
    digui(n-1,x-h,y-w/2-1,nh,nw);///上方///最右下角坐标     一只讲三点坐标投向下一个图形打印的函数中处理，直到找到每一个n=2时的坐标
}                   /*                      /  |  \                    */
...
//        return;
//    }
//    int nh = pow(2,n-2)/2,nw = (pow(2,n-1)-1)/2;//nh表示上一个图形的高度计算方法，因此为n-1-1，nw为上一个小图形的宽度计算方法
//    /这是新图形的高和宽
...
//        int h = x/2,w = y/2;//h代表标准点坐标，也就是1/2的高度，w代表标准点坐标，也即是1/2宽度
//        print(n,x,y,h,w);//放入print函数中递归处理
//        for(int i = 1; i <= x; i++)
//        {
//            for(int j = 1; j <=pow(2,n)-1; j++)  ///这里是为了控制不输出多余的空格
//            {
//                if(mp[i][j]=='*')
//                    stop=j;
//            }
