...
void insert(char S[])///建树
{
    ...
    for(int i=0;i<len;i++)
    {
        if(!Tree[now].next[S[i]-'a']) Tree[now].next[S[i]-'a']=++tot;
...
void build()///广搜建立fail树,fail是一个失配的利用，当在某个节点失配时，可以利用失配前的一串字符作为最大前缀，往后继续匹配串T，实现线性的遍历串T进行匹配
{
    ...
    while(!q.empty())
    {
        ...
        for(int i=0;i<26;i++)///队首元素出队后遍历26个孩子
        {
            if(Tree[top].next[i])
            {
                if(top==0) Tree[Tree[top].next[i]].fail=0;///根节点的直接孩子fail为0
                else///如果不是根节点，找当前节点的父亲的fail父亲...直到找到一个父亲的儿子等于下一个字母i的值，这个过程是找最大前缀
                {
                    ...
                    while(tmp!=-1)///以回溯到根节点为止
                    {
                        ...
                        {
                            Tree[Tree[top].next[i]].fail=Tree[tmp].next[i];///即找到最大前缀。
                            break;///最大前缀
                        }
...
int match(char S[])///对比查询函数
{
    ...
    {
        if(Tree[now].next[S[i]-'a']) now=Tree[now].next[S[i]-'a'];///类似字典树的匹配，如果当前字符符合字典树的节点字符，往下继续匹配
        else///如果失配，那么从失配节点找他的fail，也就从字典树中的某个单词，跳到另一个单词上继续匹配。这要使别的单词的前缀和失配单词的最大前缀相符。
        {
            int tmp=Tree[now].fail;
            while(tmp!=-1&&Tree[tmp].next[S[i]-'a']==0) tmp=Tree[tmp].fail;///找到一个符合条件的fail，符合条件的fail既要前缀相同，而且前缀的下一个字母(子节点)要符合串T正在匹配的那个字母
            if(tmp==-1) now=0;///如果向上找fail没有，那么只能跳到根节点。
            else now=Tree[tmp].next[S[i]-'a'];///否则now跳到了字典中另一个单词的节点中。
        }
        if(Tree[now].cnt) ans+=sum(now);
    }
    return ans;
}
