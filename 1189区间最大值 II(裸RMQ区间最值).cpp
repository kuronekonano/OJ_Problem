...
int a[100005];
int rmq_max(int l,int r)///最大值查找函数,输入范围的起点L和终点R，这里表示的是数组的位置而不是数的值
{
    int k=log2(r-l+1);
...
//    }
    return max(dmax[l][k],dmax[r-(1<<k)+1][k]);///起始点的前半段最值就是第l行k列，终止点后半段的最值，同样是k列，但是行数是r-范围，结果
...
int main()
{
    ...
    while(scanf("%d",&n)!=EOF)
    {
        ...
        {
            for(int i=0; i+(1<<j)-1<=n; i++)///第0列存储的是原数组，也是初始化的dp入口,第1列开始记录两两中的最值，第2列记录四个四个的最值，获取方法是从上一列中的最值中挑选，这样就能得到两段中的最值并继承下去
            {
                dmax[i][j]=max(dmax[i][j-1],dmax[i+(1<<(j-1))][j-1]);///以求得下一整段中的最值
        ...
        int l,r;///但范围，是由j外层循环决定的，外层的2的j-1次方，表示增长的范围，当范围加上i时就表示终点的位置
//        for(i=0;i<=n;i++)
//        {
//            for(int j=0;j<=n;j++)
//            {
