...
{
    snum[2]=1;//2为素数；
    for(int i=3; i<=40; i+=2)//找出40以内所有素数,素数一定是奇数，这样找可以减少遍历的量
    {
...
int dfs(int num)//从第二个数开始遍历，之后每次投入的num是指搜索到第几个数
{
    ...
    for(int i=2; i<=n; i++)
    {
        if(!vis[i] && snum[orz[num-1]+i])//若未被标记的数字，且这个数为素数，能进来的数表示都是选好了的，每次找的条件是
        {
            vis[i]=1;//标记这个数
            orz[num]=i;//将这个符合条件的数装入素数环数列数组中
            dfs(num+1);//执行这个语句时，是递归的，若能执行到最后，才会执行下一步取消标记，或者当这一行无法进行到最后时，才会执行取消标记往前退回
            vis[i]=0;//取消标记
        }
    }
    return 0;
