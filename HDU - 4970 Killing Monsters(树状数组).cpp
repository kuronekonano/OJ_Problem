...
#define LL long long///因此当sum函数求和时，从1到x的计数，其他区间的伤害将被边界的-Z抵消，而到达X时因为还未到达边界-Z抵消，所以将得到此处格子的伤害，即使重叠覆盖的区间也能得到伤害
int n,m;///attact数组表示从格子pos处到达终点的总伤害，计算时从n倒着求到1，对于 每个POS的总伤害，由i-1加sum(i)递推而来
...
int main()
{
    ...
    while(scanf("%d",&n)!=EOF)
    {
        ...
        LL hp;
        scanf("%d",&k);
        while(k--)///最终输入每个怪物的HP和出生点时，只需比较该怪物的血量是否能承受从出生点到终点的伤害总和，即可判断该怪物是否能存活
        {
            scanf("%lld%d",&hp,&pos);
