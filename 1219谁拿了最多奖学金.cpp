...
bool cmp(student a, student b) {
  if (a.fin == b.fin) {
    return a.num >
           b.num; /// 当奖金相同时按照先后顺序标码排序，此处应标码越小越先，排在越右边
  }
...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    ...
    for (i = 0; i < n; i++) {
      stu[i].fin = 0;
      stu[i].num =
          i; /// 此处有个坑，当最高奖金一样时，按照输入先后顺序来排序，此处赋值是为先后顺序标号
      scanf("%s %d %d %c %c %d", stu[i].name, &stu[i].test, &stu[i].mark,
            &stu[i].off, &stu[i].west, &stu[i].lit);
    ...
    printf("%s\n",
           stu[n - 1]
               .name); /// 输出的是第n-1个人的信息，也就是说n-1个人是奖金最多的
    printf("%d\n", stu[n - 1].fin);
    printf("%d\n", sum);
  }
  return 0;
